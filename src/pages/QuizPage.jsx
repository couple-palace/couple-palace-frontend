...
const QuizPage = () => {
  ...
  return (
    <div className="w-full flex flex-col items-center px-4 py-6 text-center">
      {/* Updated: Fixed container with extra padding and higher z-index */}
      <div className="fixed inset-x-0 top-0 bg-white z-[9999] p-2">
        <ProgressBar progress={currentQuestion + 1} total={quizData.length} />
      </div>
      
      {/* Adjust content margin-top to avoid overlap */}
      <div className="relative w-full max-w-[400px] mt-24">
        <AnimatePresence mode="wait">
          <motion.div
            key={currentQuestion} // ✅ 질문 변경 시 애니메이션 적용
            initial={{ opacity: 0, x: 50 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -50 }}
            transition={{ duration: 0.4, ease: "easeInOut" }}
            className="absolute top-0 left-0 w-full bg-white shadow-md rounded-xl border-2 border-gray-200 px-6 py-8"
          >
            {/* ✅ 여기에서 숫자 제거 (1/18 같은 표시 안 나옴) */}
      ...
      <div className="relative w-full max-w-[400px] mt-6">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="absolute top-0 left-0 w-full flex flex-col space-y-6"
          >
